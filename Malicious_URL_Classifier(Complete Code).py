...
class RetrieveData():
    ...
    def get_malicious_urls(self):
       # rw_dombl = self.dombl
        ...
        #urllib.request.urlretrieve(rw_dombl, r'F:\Python Programs(Machine Learning)\Dataset\Unzipped\Malicious_URL_Detection\rw_dombl.txt')
        ...
        # line 7.
       # lines = open(r'F:\Python Programs(Machine Learning)\Dataset\Unzipped\Malicious_URL_Detection\rw_dombl.txt').readlines()
#        open(r'F:\Python Programs(Machine Learning)\Dataset\Unzipped\Malicious_URL_Detection\rw_dombl.txt', 'w').writelines(lines[7:])

        # Creates a panda dataframe and reads in the ransomware domain
        # blocklist.
 #       df_dombl = pd.read_csv(r'F:\Python Programs(Machine Learning)\Dataset\Unzipped\Malicious_URL_Detection\rw_dombl.txt', header=None)
    ...
    def get_benign_urls(self):  
        try:
            ...
            with zipfile.ZipFile(r'F:\Python Programs(Machine Learning)\Dataset\Unzipped\Malicious_URL_Detection\top1m.csv.zip') as zip_ref:
                zip_ref.extractall(r'F:\Python Programs(Machine Learning)\Dataset\Unzipped\Malicious_URL_Detection\top1m.csv.zip')
...
def train_model():
   ...
   # clf.fit(X_train, y_train)
    #Performing Accuracy Score
   # clf_predictions = clf.predict(X_test)   
   # print("***** Accuracy Score of model: ",accuracy_score(y_test, clf_predictions))
    #Confusion Matrix
   ...
   # print("***** Cross Validation Score: ", round(score.mean()*100, 2), '%')
    # Calculate f1 score.
   ...
   # print(clf.feature_importances_)
    ...
    #Testing the fit model on testing Data
 ...
 #Accuracy Score of all the trained model
   ...
   # 
    ...
    print('*****Accuracy of Logistic Regression:',score_regression)
    #Confusion Matrix
    print("***** Confusion Matrix:\n",confusion_matrix(y_test,y_perdict_regression)) 
    # Perform cross validation and print out accuracy.
     
    score = model_selection.cross_val_score(log_reg, X_test, y_test, cv=10)
    print("***** Cross Validation Score: ", round(score.mean()*100, 2), '%')

    # Calculate f1 score.
    y_train1_pred = cross_val_predict(log_reg, X_train, y_train, cv=3)
    f = f1_score(y_train, y_train1_pred)
    print("**** F1 Score: ", round(f*100, 2), '%')
